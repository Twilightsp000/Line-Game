...
########判断是否相连
func if_connected(prev_pos_clicked, pos_clicked):
	var myoffsets = [Vector2i(0,-1), Vector2i(0,1), Vector2i(+1,-1), Vector2i(-1,1),Vector2i(1,0), Vector2i(-1,-0)]
	for offset in myoffsets:
		if prev_pos_clicked - pos_clicked == offset:
			return true
		
	print("The cha zhi shi" + str(prev_pos_clicked - pos_clicked))
	return false
